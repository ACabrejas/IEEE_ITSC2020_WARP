...
Traffic profiles contain, for each minute of a day in a given date, the expected travel time to transverse the link to which it is associated. These profile values are published together with the measured travel times, but the methodology followed to calculate them is not publicly available.
...
\subsection{Examples of Travel Times} 
The most direct variable for measuring the state of traffic over a length of road is the vehicles' travel times. 
This is normally measured by using induction loops below the lanes, which measure the instantane
...
\begin{figure}[htbp]
	...
	\caption{Measurements of travel time in link 1170079 of the M6 over a period of 28 days.}
	\label{fig:travel_time}
\end{figure}

\begin{figure}[htbp]
	\centerline{\includegraphics[width=\linewidth]{./images/ACF_M6_Link3.pdf}}
	\caption{Autocorrelation function of the Travel Time series introduced in figure \ref{fig:travel_time} with a maximum lag of 4 weeks.}
	\label{fig:ACF}
\end{figure}
...
As it can be seen in Figure \ref{fig:travel_time} the travel time remains within a vaguely predictable pattern most of the days, with bounded minima during nights corresponding with the free flow time and with some outliers below this value corresponding to speeding drivers. 

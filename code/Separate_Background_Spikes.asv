...
for j=1:length
    ...
    saveas(gcf, strcat('./03_background_spikes_matlab_to_r/',motorway,'_link_',string(j),'.png'));
    hold off;
    plot(reconstructed-travel_time');
    legend('Reconstructed - Travel Time');
    saveas(gcf, strcat('./03_background_spikes_matlab_to_r/',motorway,'_link_',string(j),'_difference_tt_reconstruction.png'));
